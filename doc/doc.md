...
### 四种功能的实现方式：
* -w：以26个字母分别为首字母，调用DFS函数，deep表示搜索的深度，每次递归调用DFS时加1，也即目前的单词链长度，每次发现更长的单词链时，更新最长的长度和最长单词链
* -c：改变变量deep的含义，使每次递归调用时其增量为当前单词的字母个数
* -h、-t：选择-h时，仅对指定的首字母调用DFS函数;选择-t时，仅在尾字母与指定字母相同时更新最长单词链和长度
* -n：设置一个阈值，等于-n指定的长度，DFS的深度在到达这个阈值时即返回，即不搜索长度大于n的链，当找到一个符合条件的单词链时，立即将其输出到文件，为单词链的总数设置计数，在最后写入文件开头
```C++
